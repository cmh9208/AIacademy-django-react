...
class UserService(object):
    ...
    def create_user(self) -> []:
        userid = shortuuid.ShortUUID(alphabet=string.ascii_lowercase + string.digits).random(length=8)
        email = str(lambda_string(4)) + "@test.com"
        ...
        return userid, email, password, username, phone, birth, address, job, interests

    def create_users(self) -> []:
        rows = [self.create_user() for i in range(100)]
        columns = ['userid', 'email', 'password', 'username', 'phone', 'birth', 'address', 'job', 'interests']
