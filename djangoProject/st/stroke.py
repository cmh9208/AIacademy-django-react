...
class Stroke:
    def __init__(self):
        self.stroke = pd.read_csv(os.path.join(root, "st", "data", "healthcare-dataset-stroke-data.csv"))
        self.my_stroke = None
    ...
    def norminal(self):
        ...
        self.spec()
        print(" ### 프리프로세스 종료 ### ")
        self.stroke.to_csv(os.path.join(root, "st", "data", "stroke.csv"), index=False)
    ...
    def set_target(self):
        df = pd.read_csv(os.path.join(root, "st", "data", "stroke.csv"))  # Unnamed: 0 컬럼생성방지
